...
def process_one_step(step_value, df_step, k=60):
    """
    EXACT SAME LOGIC as your single-step script, just wrapped for one step.
    Uses your column names: atom_id, mol_id, S.
    """
    ds = df_step.copy()
    ...
    for i in range(len(coords)):
        ...
        for j in indices[i]:
            if i != j:
                aids_j = aids[j]
                mid_j = mids[j]
                # same exclusion rule: different chain OR |Δatom_id| > 4
