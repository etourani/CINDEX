...
with open('dumpH_t2.csv', 'w') as fl:
    tmp= fl.write('step,t,atom_id,mol_id,x,y,z,pe,h,xlo,xhi,ylo,yhi,zlo,zhi\n')
    for step in range(Ns):
        ...
        for i in range(N):
            ...
            atom_pe= round_to_precision(float(lines[idx].split(' ')[5]))
            atom_h = round_to_precision(float(lines[idx].split(' ')[6]))
            ...
            if i == 0:
                tmp= fl.write(str(step)+','+str(t)+','+str(atom_id)+','+
                          ...
                          str(atom_z)+','+str(atom_pe)+','+str(atom_h)+','+str(xlo)+','+str(xhi)+','+str(ylo)+','+str(yhi)+','+str(zlo)+','+str(zhi)+'\n')
            else:
                tmp = fl.write(str(step)+','+str(t)+','+str(atom_id)+','+ 
                          str(mol_id)+','+str(atom_x)+','+str(atom_y)+','+
                          str(atom_z)+','+str(atom_pe)+','+str(atom_h)+','+str(0)+','+str(0)+','+str(0)+','+str(0)+','+str(0)+','+str(0)+'\n')
