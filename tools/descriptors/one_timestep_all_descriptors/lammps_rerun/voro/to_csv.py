...
with open('voro_t2.csv', 'w') as fl:
    tmp= fl.write('step,t,atom_id,mol_id,v,nfaces,xlo,xhi,ylo,yhi,zlo,zhi\n')
    for step in range(Ns):
        ...
        for i in range(N):
            ...
            atom_v= float(lines[idx].split(' ')[2])
            n_faces= float(lines[idx].split(' ')[3])



            if i == 0:
                tmp= fl.write(str(step)+','+str(t)+','+str(atom_id)+','+
                          str(mol_id)+','+str(atom_v)+','+str(n_faces)+',' +str(xlo)+','+str(xhi)+','+str(ylo)+','+str(yhi)+','+str(zlo)+','+str(zhi)+'\n')
            else:
                tmp = fl.write(str(step)+','+str(t)+','+str(atom_id)+','+
                          str(mol_id)+','+str(atom_v)+','+str(n_faces)+','+str(0)+','+str(0)+','+str(0)+','+str(0)+','+str(0)+','+str(0)+'\n')
